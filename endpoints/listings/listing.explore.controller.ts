...
export async function toggleWishlist(req: Request & { user?: any }, res: Response) {
  try {
    ...
    const existing = await db.wishlistItem.findFirst({
      where: { userId, listingId, categoryId: cat.id },
    });
    ...
    await db.wishlistItem.create({
      data: { userId, listingId, categoryId: cat.id },
    });
...
export async function getWishlist(req: Request & { user?: any }, res: Response) {
  try {
    ...
    const items = await db.wishlistItem.findMany({
      ...
      orderBy: { createdAt: 'desc' },
      include: { listing: { include: { _count: { select: { wishlistedBy: true } } } } },
    });
...
export async function toggleWishlistInCategory(req: Request & { user?: any }, res: Response) {
  try {
    ...
    const existing = await db.wishlistItem.findFirst({
      where: { userId, listingId, categoryId },
    });
    ...
    await db.wishlistItem.create({ data: { userId, listingId, categoryId } });
    return response({ res, code: 200, success: true, msg: 'ok', data: { wishlisted: true } });
  } catch (err: any) {
    return response({ res, code: 500, success: false, msg: err?.message || 'server_error' });
  }
...
export async function getListingDetails(req: Request, res: Response) {
  try {
    ...
    const listing = await db.listing.findUnique({
      where: { id: listingId },
      include: {
        host: { select: { id: true, name: true, picture: true, role: true } },
    ...
    const isGuestFavorite = Number(listing?._count?.wishlistedBy ?? 0) >= threshold;
    const outListing = { ...listing, isGuestFavorite };
    delete (outListing as any)._count;
    return response({ res, code: 200, success: true, msg: 'ok', data: { listing: outListing, reviewCount } });
  } catch (err: any) {
    return response({ res, code: 500, success: false, msg: err?.message || 'server_error' });
  }
}
